...
$.ajax({
        type: 'GET',
        url: 'https://api.swordpal.cn/currentWeatherImg',
        data: param,
        dataType: 'json',
        error: function () {
            console.log("获取天气异常");
        ...
        success: function (res) {
            console.log("调用结束"+res.img);
           //weatherImg=res.img;
           weatherImg="https://img.swordpal.cn/imgs/qing.png";
        
        //var img = new Image();
        //console.log("当前节气：" + weatherImg)
         //img.src = "https://img.swordpal.cn/imgs/" + seasonal + ".png";
        //img.src=weatherImg;//"https://img.swordpal.cn/imgs/winter.png";
        //img.onload = function () {
        //     startSakura();
       // }
           //startSakura();
           staticWeather(weatherImg);
...
function startSakura() {
    ...
    var canvas =document.createElement('canvas'),
        cxt;
    staticx = true;
    canvas.height = document.getElementById('page-header').offsetHeight;
...
function staticWeather(weatherImg) {
    var canvas = document.createElement('canvas');
    staticx = true;
    let height = document.getElementById('page-header').offsetHeight;
    let width = window.innerWidth;
    canvas.height = height;
    canvas.width = width;
    canvas.setAttribute('style', 'position: absolute;left: 0;top: 0;pointer-events: none;z-index: 99;');
    canvas.setAttribute('id', 'sun');
    document.getElementsByTagName('body')[0].appendChild(canvas);
    ...
    img.onload = function() {
        ctx.drawImage(img, width / 2, height / 7)
    }
}
